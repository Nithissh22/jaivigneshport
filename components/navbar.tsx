...
export default function Navbar() {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Determine active section based on scroll position (supports lazy-loaded sections)
    let current = activeSection
    for (const item of navItems) {
      const el = document.getElementById(item.href.substring(1))
      if (!el) continue
      const rect = el.getBoundingClientRect()
      // Active if the section overlaps the middle of the screen
    ...
    // Also handle being at the very top or very bottom
    if (latest === 0) current = "home"
    if (window.innerHeight + latest >= document.body.scrollHeight - 50) current = "contact"
